...
df_log = pd.read_csv(
    ...
    header=None, 
    names=('Date', 'Time', 'CS', 'Band', 'Mode', 'His', 'My', "Remarks"),
    index_col=0
)

#ACAGなどパワーがMyについている場合は削除
#CWとPhoneで599/59を削除、出力の行末を削除
#astype(str)で文字列に
df_log['My'] = df_log['My'].astype(str).replace('(^[1-5][1-9]9?)(\d.*?)([HLMP]?$)', r'\2', regex=True)
...
df_My.insert(2, 'JCGmuniCd', df_My['JCCJCGd'].map(df_JCG.set_index('JCGCd')['muniCd']))   
...
#print(df_My['JCCmuniCd'][df_My['JCCmuniCd'].duplicated()])
...
#緯度経度指定で描画エリア設定。全国地図で大きすぎる、色が薄すぎる場合にはマルチの範囲でgooglemapでもみて範囲を見て設定してください。現在は関東地方にセットしてます。
plt.xlim([138,141])
plt.ylim([35,37])
